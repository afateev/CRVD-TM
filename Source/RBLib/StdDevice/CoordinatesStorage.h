///////////////////////////////////////////////////////////////////////////////
//	Класс хранения и выдачи географических координат устройства
//	
///////////////////////////////////////////////////////////////////////////////

#ifndef COORDINATESSTRORAGE_H
#define COORDINATESSTRORAGE_H
...
///////////////////////////////////////////////////////////////
///	\brief Структура для хранения координат устройства в энергонезависимой памяти
///	
///////////////////////////////////////////////////////////////
struct CoordinatesStorageDataType
{
	struct DataStruct
	{
		float Lat;			// Широта
...
///////////////////////////////////////////////////////////////
///	\brief Класс хранения и выдачи географических координат устройства
///	
...
public:
	...
	CoordinatesStorage() : _myNumber(_myNumberInvalid)
	{
	...
	*/
	void CoordinatesStorageSetMyNumber(unsigned char myNumber)
	{
		_myNumber = myNumber;
	}
	...
	*/
	void CoordinatesStorageCommandHandler(Rblib::Protocol::CmdInfo *cmd, Rblib::Communication::InterfaceBase *outputInterface)
	{
		switch(cmd->Cmd)
		{
		// Запрос координат
		case Rblib::Protocol::CmdGetCoordinates:
			{
				...
				if (0 != cmd->Data && cmd->DataSize >= requiredDataSize)
				{
					...
					if (_myNumber == deviceNumber)
					{
						...
						if (GetFromContainer(Lat, Lon))
						{
							...
							_cmdData.Lon = Lon;
						  // команда ответа
							answer.Cmd = Rblib::Protocol::CmdNotifyCoordinates;
							// поле данных
			...
			break;
		// Установка координат
		case Rblib::Protocol::CmdSetCoordinates:
			{
				// если не знаем свой номер, то ничего не можем сделать
				if (_myNumberInvalid == _myNumber)
					return;
				...
				answer.Cmd = Rblib::Protocol::CmdSetCoordinatesNak;
				// если есть данные и размер подходящий
				if (0 != cmd->Data && cmd->DataSize >= requiredDataSize)
				{
					// получаем данные
					...
					if (_myNumber == _cmdData.DeviceNumber)
					{
						...
						// ответ "успешно"
			  			answer.Cmd = Rblib::Protocol::CmdSetCoordinatesAck;
...
protected:
	...
	{
		// Читаем
		CoordinatesStorageDataType data = Container::Read();
	...
	void SetToContainer(float Lat, float Lon)
	{
		...
		CoordinatesStorageDataType data;
