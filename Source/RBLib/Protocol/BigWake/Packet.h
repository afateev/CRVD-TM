///////////////////////////////////////////////////////////////////////////////
//	Формат пакета BigWAKE
//
///////////////////////////////////////////////////////////////////////////////

#ifndef PROTOCOL_BIGWAKE_PACKET_H
#define PROTOCOL_BIGWAKE_PACKET_H
...
#pragma pack(push, 1)
	...
	public:
		...
		Packet(void *data, unsigned int dataLen)
		{
		...
		Packet(unsigned char command, void *data, unsigned int dataLen)
		{
			Set(command, data, dataLen);
		}
		
		// Конструктор по полю команды и полю данных
		void Set(unsigned char command, void *data, unsigned int dataLen)
		{
			_dataCount = 0;
			...
			_data[2] = dataLen & 0xFF;
			_data[3] = (dataLen >> 8) & 0xFF;
			if (dataLen > 0)
			{
		...
		Packet(unsigned char address, unsigned char command, void *data, unsigned int dataLen)
		{
			Set(address, command, data, dataLen);
		}
		
		// Конструктор по полю адреса, полю команды и полю данных
		void Set(unsigned char address, unsigned char command, void *data, unsigned int dataLen)
		{
			_dataCount = 0;
			int packetLen = 5 + dataLen + 1;
			...
			_data[3] = dataLen & 0xFF;
			_data[4] = (dataLen >> 8) & 0xFF;
			if (dataLen > 0)
			{
				for (int i = 0; i < dataLen; i++)
				{
					_data[5 + i] = ((unsigned char *)data)[i];
				}
			}
			_data[packetLen - 1] = CalcCrc();
		}
		...
		bool HaveData()
		{
			int offset = DataLenOffset() + 2;
			unsigned int dataLen = DataLen();
			unsigned int expectedCount = offset + dataLen;
			
			if (_dataCount > 254)
			{
				return _dataCount >= expectedCount && dataLen > 0;
			}
	...
	protected:
				...
				case FrameStart:
					{
						...
						dstPtr[1] = TransposedFrameStart;
						return 2;
					}
				case FrameEscape:
					{
						dstPtr[0] = FrameEscape;
						dstPtr[1] = TransposedFrameEscape;
						return 2;
					}
				default:
					{
		...
		unsigned int UnStuff(unsigned char *data, unsigned int dataCount, unsigned char *res, unsigned int resSize)
        {
