...
#pragma pack (pop)
		...
		typedef CallbackWrapper<int, bool, int> TxTimeSlotChainCallbackType;
	...
	public:
		...
		void FixTime()
		{
			...
			_lastFixSlotNumber = _timeSlotChain.GetCurSlot();
...
#endif
					{
						// не надо выдавать сигнал коррекции
						use = false;
						break;
					}
				}
		...
		{
			return _timeSlotChain.GetSlotCount();
		}
	...
	protected:
		...
		static void OnTimeSlotStatic(void *callbackParam)
		{
			...
			{
				if (im->_timeSlotChain.GetCurSlot() == im->_timeSlotChain.GetSlotCount() + 1)
				{
		...
		void OnTxTimeSlot(int slot, bool mainSlot, int dataSlotCounter)
		{
			...
			if (_state == StateSync)
			{
				_onTxTimeSlotCallback.Call(slot, mainSlot, dataSlotCounter);
			}
		}
		...
		void OnRxTimeSlot(int slot)
		{
			...
			case StateSync:
				{
					_rxOnOffCallback.Call(true, true);
				}
				break;
			}
