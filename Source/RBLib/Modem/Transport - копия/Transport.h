...
#include "../Common/CallbackWrapper.h"
...
#pragma pack (pop)
		...
		static const unsigned int MaxDataCountPerSession = 1024;
		static const unsigned int InputQueueSize = 10240;
	...
	public:
		...
		int DequeueData(AddressType selfAddress, unsigned char *buffer, int bufferSize)
		{
			if (!_running)
			{
				return 0;
			...
			{
				dequeuedDataCount = _sessions[nextSession].PopNextOutputData(buffer, bufferSize, curTime);
				...
				if (SessionType::ShouldOpenSession(data, dataCount))
				{
					if (!_transportUsedTycketsBuffer.Present(inputTicket))
					{
						...
						if (session != 0)
						{
							...
							session->Init(selfAddress, sourceAddress, data, dataCount, GetRouteCallback, RxDoneCallback, DebugDataCallback, this);
						}
						else
						{
							//TODO
