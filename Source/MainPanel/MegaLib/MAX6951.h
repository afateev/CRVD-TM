...
public:
	static void Init()
	{
		...
		WriteReg(regScanLimit, 7);
		Test(false);
		Clear();
	}
	
	static void Enable(bool enable = true, bool blink = false)
	{
		unsigned char flEnable = enable ? 1 : 0;
        unsigned char flBlink = (blink ? 1 : 0) << 3;
        WriteReg(regConfig, flEnable | flBlink);
	}
