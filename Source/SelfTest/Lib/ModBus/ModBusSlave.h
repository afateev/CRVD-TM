...
public:
	...
	typedef bool (SetRegValueCallbackType)(unsigned short regAddr, unsigned short value);
	static GetRegValuesCallbackType *GetRegValuesCallback;
	static SetRegValueCallbackType *SetRegValueCallback;
public:
	...
	static unsigned int OnRequest(unsigned char *data, unsigned int dataLen)
	{
		...
		case 0x06:
				return OnWriteSingleRegister(data, dataLen, ModBusStateMachine::BufferLen);
		default:
	...
	static unsigned int OnWriteSingleRegister(unsigned char *data, unsigned int dataLen, unsigned int bufferLen)
	{
		if (!SetRegValueCallback)
		{
			return OnNotSupported(data, bufferLen);
		}
		...
		unsigned short regAddr = data[2];
		regAddr <<= 8;
		regAddr |= data[3];
		
		unsigned short val = data[4];
		val <<= 8;
		val |= data[5];
		
		if (SetRegValueCallback(regAddr, val))
		{
			return dataLen;
		}
		else
...
template <class ModBusStateMachine>
typename ModBusSlave<ModBusStateMachine>::SetRegValueCallbackType *
ModBusSlave<ModBusStateMachine>::SetRegValueCallback = 0;
